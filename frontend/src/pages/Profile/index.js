...
export default function Profile() {
  ...
  useEffect(async () => {
    try {
      const response = await BlocksService.listScheduled(
  ...
  const fetchLabs = useCallback(async (block) => {
    try {
      const response = await LabsService.listScheduled(
        block.id,
        localStorage.getItem('teacher')
      );
      if (response.status === 200 && !response.data.erro) {
  ...
  const fetchSchedules = useCallback(async (lab) => {
    try {
      const response = await ScheduleService.getScheduleByTeacher(
        lab.laboratory_id,
        localStorage.getItem('teacher')
      );
  ...
  return (
    <ContainerPage>
      ...
      <ContainerCalendar>
        <Calendar scheduled={schedule} selected={selected} profile />
      </ContainerCalendar>
    </ContainerPage>
  );
}
